...
if ($isTumor)
{
  ...
  my $mPileup  = `cd $param->{'samPath'}\n ./samtools mpileup -B -R -r $block -l $param->{'regionsFile'} --rf 0x2 --ff 0x400 -O -s -Q 1 -C $param->{'mpileupC'} -f $param->{'refGenome'} $param->{'bamFile'}`;
  #my $output;
...
else
{
  ...
  my $mPileup  = `cd $param->{'samPath'}\n ./samtools mpileup -r $block -l $param->{'regionsFile'} --rf 0x2 --ff 0x400 -O -s -Q 1 -C $param->{'mpileupC'} -f $param->{'refGenome'} -b $param->{'bamList'}`;
