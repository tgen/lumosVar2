...
sub parse()
{
  ...
  foreach my $line (@pileupLines)
  {
    ...
    elsif($pileupData->[$i-1]->{'Pos'}!=$pos-1)
    {
      ...
      @currSeqIdx=($nextSeqIdx...$nextSeqIdx+$readCount);
      $nextSeqIdx+=$readCount+1;
    }
	...
	if($currBQ==0)
        ...
        elsif($currBQ>=$param->{'minBQ'} && $currMQ>=$param->{'minMQ'} && $b=~/([AGCTagct])/)
        {
	...
	if($currBQ==0)
	  ...
	  $overlapBaseCount++;
        ...
        elsif($currBQ>=$param->{'minBQ'} && $currMQ>=$param->{'minMQ'})
        {
    ...
    $pileupData->[$i]->{'readCount'}=$readCount-$overlapBaseCount;
    $pileupData->[$i]->{'readCountPass'}=$readCount-$lowQCBaseCount-$overlapBaseCount;
