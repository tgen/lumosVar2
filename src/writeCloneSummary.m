...
if inputParam.NormalSample<1
    ...
    plotSpread(min(sampleFrac(somIdx,:),1),'CategoryIdx',repmat(cloneIdplot(somIdx),1,size(sampleFrac,2)),'CategoryColors',[0 0 0; colors])
end
set(gca,'XTick',1:size(f,1),'XTickLabel',sampleNames,'FontSize',8,'TickLabelInterpreter','none','XTickLabelRotation',10);
ylabel('Variant Sample Fraction');
...
text(zeros(size(sampleNamesShort')),linspace(-0.1*ylm(2),-0.1*ylm(2)*length(sampleNamesShort),length(sampleNamesShort)),sampleNamesShort,'HorizontalAlignment','right','Interpreter','none','FontSize',8);
yt=get(gca,'YTick');
