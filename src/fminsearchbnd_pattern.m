function [x,fval,exitflag,output] = fminsearchbnd_pattern(fun,x0,LB,UB,options,varargin)
...
[xu,fval,exitflag,output] = patternsearch(@(x)intrafun(x,params),x0u,[],[],[],[],[],[],[],options);
%problem = createOptimProblem('patternsearch','objective',@(x)intrafun(x,params),'x0',x0u,'options',options);
%ms=MultiStart('UseParallel',1,'Display','iter');
%[xu,fval,exitflag,output]=patternsearch(problem);
