function [segsTable, W, f, c, nll, pCNA, V]=fitCNA_v2(dataHet,dataSom,exonRD,segs,priorF,inputParam)
...
[param, nll] = fmincon(@(param)nllCNA_v2(dataHet,dataSom,exonRD,segs,priorF,inputParam,param),[cr nanmedian(exonRD(:,4)*ones(1,inputParam.numClones)) 1/(inputParam.numClones+1):1/(inputParam.numClones+1):1-1/(inputParam.numClones+1)],[],[],[],[],[0.5*cr inputParam.minW*ones(1,inputParam.numClones) zeros(1,inputParam.numClones)],[2*cr inputParam.maxW*ones(1,inputParam.numClones) ones(1,inputParam.numClones)],[],opts);
c=param(1);
