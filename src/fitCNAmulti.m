function [segsTable, W, f, CNAscale, nll, t]=fitCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,fInit,cInit,wInit,dbCounts)
%fitCNAmulti - uses EM to fit copy number parameters and estimate copy number
%
% Syntax: [segsTable, W, f, CNAscale, nll, t]=fitCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,fInit,cInit,wInit,dbCounts)
%
% Inputs:
...
[param{j}, nll(j)]=fmincon(@(param)nllCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,param,dbCounts),[100.*cInit(:); wInit(:); 100*fInit(:)],[],[],[],[],[50*cInit(:); inputParam.minW*ones(size(wInit(:))); zeros(size(fInit(:)));],[200*cInit(:); inputParam.maxW*ones(size(wInit(:))); 100*ones(size(fInit(:)));],[],opts2);
bestMin=nll(j)+j*length(tIdx)./inputParam.addCloneWeight;
prevMin=bestMin;
CNAscale=param{j}(1:inputParam.sampleCount)./100
Wcurr=param{j}(inputParam.sampleCount+1:2*inputParam.sampleCount)
fOld=reshape(param{j}(2*inputParam.sampleCount+1:end),[],inputParam.numClones)
cBest=CNAscale;
wBest=Wcurr;
fBest=fOld;
iterStuck=0;
while iterStuck<inputParam.iterNoImp
    ...
    parfor k=1:size(cTest,2)
        nllRecenter(k)=nllCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,[100.*cTest(:,k); Wcurr(:); fOld(:)],dbCounts);
    end
    [~,cIdx]=min(nllRecenter);
    if(any(cTest(:,cIdx)~=CNAscale))
        [paramRecenter, nllRecenter(cIdx)]=fmincon(@(param)nllCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,param,dbCounts),[100.*cTest(:,cIdx); Wcurr(:); fOld(:)],[],[],[],[],[50*cTest(:,cIdx); inputParam.minW*ones(size(wInit(:))); zeros(size(fOld(:)));],[200*cTest(:,cIdx); inputParam.maxW*ones(size(wInit(:))); 100*ones(size(fOld(:)));],[],opts2);
        currMin=nllRecenter(cIdx)+j*length(tIdx)./inputParam.addCloneWeight;
        message=['Recentering']
        CNAscale=paramRecenter(1:inputParam.sampleCount)./100
        Wcurr=paramRecenter(inputParam.sampleCount+1:2*inputParam.sampleCount)
        fOld=reshape(paramRecenter(2*inputParam.sampleCount+1:end),[],inputParam.numClones)
        if currMin<bestMin
            foundMin=1;
            bestMin=currMin
            cBest=CNAscale;
    ...
    while removeClone && j>=2
        ...
        parfor i=1:j
            idx=setdiff(1:j,i);
            fRemove{i}=fOld(:,idx);
            nllRemove(i)=nllCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,[100.*CNAscale(:); Wcurr(:); fRemove{i}(:)],dbCounts);
        end
        [~,removeIdx]=min(nllRemove)
        j=j-1;
        [paramRemove, nll(j)]=fmincon(@(param)nllCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,param,dbCounts),[100.*CNAscale(:); Wcurr(:); fRemove{removeIdx}(:)],[],[],[],[],[50*cInit(:); inputParam.minW*ones(size(wInit(:))); zeros(size(fRemove{removeIdx}(:)));],[200*cInit(:); inputParam.maxW*ones(size(wInit(:))); 100*ones(size(fRemove{removeIdx}(:)));],[],opts2);
        currMin=nll(j)+j*length(tIdx)./inputParam.addCloneWeight;
        if currMin<bestMin
            foundMin=1;
            removeClone=1;
            bestMin=currMin
            CNAscale=paramRemove(1:inputParam.sampleCount)./100
            Wcurr=paramRemove(inputParam.sampleCount+1:2*inputParam.sampleCount)
            fOld=reshape(paramRemove(2*inputParam.sampleCount+1:end),[],inputParam.numClones)
            cBest=CNAscale;
            wBest=Wcurr;
            fBest=fOld;
    ...
    while addClone
        ...
        parfor i=1:size(pts,2)
            nllPTS(i)=nllCNAaddClone(hetData,somData,dbData,exonRD,segsMerged,inputParam,CNAscale,Wcurr,fOld,pts(:,i),dbCounts);
        end
        [~,idx]=min(nllPTS)
        toc
        t(j,1)=toc
	...
	fAdd=[fOld pts(:,idx)]
        tic
        [param{j}, nll(j)]=fmincon(@(param)nllCNAmulti(hetData,somData,dbData,exonRD,segsMerged,inputParam,param,dbCounts),[100.*CNAscale(:); Wcurr(:); fAdd(:)],[],[],[],[],[50*cInit(:); inputParam.minW*ones(size(wInit(:))); zeros(size(fAdd(:)));],[200*cInit(:); inputParam.maxW*ones(size(wInit(:))); 100*ones(size(fAdd(:)));],[],opts2);
        currMin=nll(j)+j*length(tIdx)./inputParam.addCloneWeight;
        t(j,2)=toc;
        if currMin<bestMin
            foundMin=1;
...
[N, M, Ftable, log2FC, cnaIdx]=callCNAmulti(hetData,exonRD,segsMerged,inputParam,paramOpt,dbData,dbCounts);
