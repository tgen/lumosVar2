...
while(true)
    ...
    [segsTable, W, f, CNAscale, nll, t{i}]=fitCNAmulti(hetPos & ~indelPos,somPos & ~indelPos,filtPos & min([Tcell{1}.ApopAFcomb Tcell{1}.BpopAFcomb],[],2)>inputParam.maxSomPopFreq,Tcell,exonRD,segsMerged,inputParam,f,CNAscale,W,dbCounts)
    for j=1:length(Tcell)
        T=Tcell{j};
        idx=getPosInRegionSplit([T.Chr T.Pos],segsTable{:,1:3},inputParam.blockSize);
        T.NumCopies=segsTable.N(idx);
...
plotCNAandVAF(exonRD,segsTable,CNAscale,fSort,Tcell,somPos,hetPos,cloneIdSort,inputParam);
for i=1:length(F)
