function nll = nllCNA_v2(dataHet,dataSom,exonRD,segs,priorFsample,inputParam,param)
...
for i=1:length(f)
    ...
    priorCNAf(cnaIdx(idx)==i,:)=betapdf(f(i),inputParam.alphaF,(inputParam.alphaF-1)./priorFsample-inputParam.alphaF+2);
end
...
if isempty(dataSom)
    ...
    priorF=betapdf(f(somIdx),inputParam.alphaF,(inputParam.alphaF-1)./priorFsample-inputParam.alphaF+2);
end
