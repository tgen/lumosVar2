function [x,fval,exitflag,output] = fminsearchbnd_unc(fun,x0,LB,UB,options,varargin)
...
%[xu,fval,exitflag,output] = fminunc(@intrafun,x0u,options,params);
problem = createOptimProblem('fminunc','objective',@(x)intrafun(x,params),'x0',x0u,'options',options);
ms=MultiStart('UseParallel',1,'Display','iter');
[xu,fval,exitflag,output]=run(ms,problem,5*length(x0u));
